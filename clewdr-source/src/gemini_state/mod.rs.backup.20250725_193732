...
use reqwest::{Client, ClientBuilder, header::AUTHORIZATION};
...
impl GeminiState {
    ...
    pub async fn request_key(&mut self) -> Result<(), ClewdrError> {
        ...
        if let Some(proxy) = CLEWDR_CONFIG.load().rquest_proxy.to_owned() {
    ...
    ) -> Result<reqwest::Response, ClewdrError> {
        let mut client = ClientBuilder::new();
        if let Some(proxy) = CLEWDR_CONFIG.load().rquest_proxy.to_owned() {
            client = client.proxy(proxy);
        }
    ...
    ) -> Result<impl Stream<Item = Result<Bytes, reqwest::Error>> + Send + 'static, ClewdrError> {
        if self.stream {
