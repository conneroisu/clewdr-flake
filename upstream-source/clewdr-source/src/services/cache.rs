...
impl ClewdrCache {
    ...
    ///
    /// # Returns
    /// * `Option<impl Stream<Item = Result<Bytes, reqwest::Error>> + Send + 'static>` - The cached response as a stream, if available
    pub async fn pop(
        &self,
        key: u64,
    ) -> Option<impl Stream<Item = Result<Bytes, reqwest::Error>> + Send + 'static> {
...
///
/// This utility function consumes a stream of `Result<Bytes, reqwest::Error>`
...
/// a stream of `Result<Bytes, reqwest::Error>` where each item is wrapped in Ok.
///
/// # Arguments
/// * `bytes` - The vector of bytes to convert to a stream
///
/// # Returns
/// * `impl Stream<Item = Result<Bytes, reqwest::Error>>` - Stream of successful byte results
fn vec_to_stream(bytes: Vec<Bytes>) -> impl Stream<Item = Result<Bytes, reqwest::Error>> {
